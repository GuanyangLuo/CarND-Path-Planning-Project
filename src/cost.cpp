...
float lane_speed(const map<int, vector<Vehicle>> &predictions, int lane, const Vehicle &ego) {
  // Find the speed of the slowest vehicle (in a specific lane) ahead of the car
  float min_v = ego.target_speed;
  ...
  for (map<int, vector<Vehicle>>::const_iterator it = predictions.begin(); 
    ...
    int key = it->first;    
    temp_vehicle = it->second[ego.prediction_index]; // Get the predicted vehicle state at a specific time
    if (temp_vehicle.lane == lane && key != -1 
        && temp_vehicle.v < min_v ) {
      min_v = temp_vehicle.v;
      found_vehicle = true;
    }
  }
